...
namespace b_118.Commands
{
    ...
    class CampaignCommands : BaseCommandModule
    {
        ...
        public async Task CreateCampaign(CommandContext ctx, [Description("Name of the campaign.")] string name, [Description("Color used for the role.")] DiscordColor? color = null)
        {
            await _prefix.Verify(ctx.Prefix, async () =>
            {
                ...
                var role = await ctx.Guild.CreateRoleAsync(name, null, color, null, false, $"Create {name} Campaign.");
                await ctx.Member.GrantRoleAsync(role, $"Creator of the {name} Campaign.");
                var campaignCategory = await ctx.Guild.CreateChannelCategoryAsync(name, GetCategoryOverwriteBuilders(ctx.Guild.EveryoneRole, role));
                var announcements = await ctx.Guild.CreateTextChannelAsync("announcements", campaignCategory, null, GetAnnouncementsOverwriteBuilders(ctx.Guild.EveryoneRole, ctx.Member, role), null, default, $"Create {name} Campaign.");
                Task.WaitAll(
                    ctx.Guild.CreateTextChannelAsync("bot-commands", campaignCategory, null, null, null, default, $"Create {name} Campaign."),
                    ctx.Guild.CreateTextChannelAsync("chat", campaignCategory, null, null, null, default, $"Create {name} Campaign."),
                    ctx.Guild.CreateTextChannelAsync("dm-notes", campaignCategory, null, GetDMNotesOverwriteBuilders(ctx.Guild.EveryoneRole, ctx.Member, role), null, default, $"Create {name} Campaign."),
                    ctx.Guild.CreateTextChannelAsync("off-topic", campaignCategory, null, null, null, default, $"Create {name} Campaign."),
                    ctx.Guild.CreateVoiceChannelAsync("sending-stones", campaignCategory, null, null, null, $"Create {name} Campaign.")
                );
        ...
        [Command("create")]
        [Hidden]
        public async Task CreateCampaign(CommandContext ctx, string name, string color)
        {
            await _prefix.Verify(ctx.Prefix, async () =>
            {
                var messaging = new Messaging(ctx);
                await messaging.RespondContent(true, TimeSpan.FromSeconds(30))($"{color} is an invalid color.");
            });
        }
        ...
        public async Task InviteToCampaign(CommandContext ctx, [Description("Name of the campaign.")] string name, [Description("Member to invite.")] DiscordMember member)
        {
            ...
            {
                var messaging = new Messaging(ctx);
                var channel = GetCampaignCategory(ctx, name);
                if (CheckOwnership(ctx, channel))
                {
                    var role = ctx.Guild.Roles.First(role => role.Value.Name.Equals(name)).Value;
                    if (member.Roles.Contains(role))
                    {
                        await messaging.RespondContent()($"{member.Nickname} is already in {name}.");
                        return;
                    }
                    var blacklistedInvites = await Program.GetB118DB().GetInviteBlacklistForUser(member.Id);
                    if (blacklistedInvites.Contains((int)role.Id))
                    {
                    ...
                    var message = await member.SendMessageAsync($"{ctx.User.Username} has invited you to join the {name} campaign."
                    ...
                    var reaction = await interactivity.WaitForReactionAsync((e) =>
        ...
        public async Task JoinCampaign(CommandContext ctx, [Description("Name of the campaign to join.")] string name)
        {
            await _prefix.Verify(ctx.Prefix, async () =>
            {
                ...
                var blacklistedRequests = await Program.GetB118DB().GetInviteBlacklistForCampaign(role.Id);
                var channel = GetCampaignCategory(ctx, name);
                var dm = await GetOwner(ctx, channel);
                if (blacklistedRequests.Contains((int)ctx.Member.Id))
                {
                ...
                var checkmark = DiscordEmoji.FromName(ctx.Client, ":white_check_mark:");
                var cancel = DiscordEmoji.FromName(ctx.Client, ":x:");
                var message = await dm.SendMessageAsync($"{ctx.User.Username} has requested to join your {name} campaign."
        ...
        public async Task DeleteCampaign(CommandContext ctx, [Description("Name of the campaign to delete.")] string name)
        {
            await _prefix.Verify(ctx.Prefix, async () =>
            {
                ...
                if (CheckOwnership(ctx, channel))
                {
                    var role = ctx.Guild.Roles.First(role => role.Value.Name.Equals(name));
        ...
        public async Task RenameCampaign(CommandContext ctx, [Description("Name of the campaign to rename.")] string currentName, [Description("New name for the campaign.")] string nextName)
        {
            ...
            {
                var messaging = new Messaging(ctx);
                var channel = GetCampaignCategory(ctx, currentName);
                if (CheckOwnership(ctx, channel))
                {
                    var nextChannel = GetCampaignCategory(ctx, nextName);
                    var nextRole = ctx.Guild.Roles.FirstOrDefault(role => role.Value.Name.Equals(nextName));
                    if (nextChannel == null && nextRole.Value == null)
                    {
                        var action = new Action<DSharpPlus.Net.Models.ChannelEditModel>((target) =>
                        {
                            target.Name = nextName;
                        });
                        await channel.ModifyAsync(action);
                        var role = ctx.Guild.Roles.FirstOrDefault(role => role.Value.Name.Equals(currentName));
